...
public class TAttendanceController {
    ...
    public List<Integer> getSingleDay(@PathVariable(value="grade")int curGrade,
        ...
        if(cDate.equals(localDate)){
            if(day != 1) localDate = localDate.minus(1, ChronoUnit.DAYS);
            else localDate = localDate.minus(3,ChronoUnit.DAYS);
    ...
    public List<List<Integer>> getWeeklyChart(@PathVariable(value="curGrade")int curGrade,
        ...
        int thisWeek = localDate.getDayOfMonth() / 7;
        int pickedWeek = cDate.getDayOfMonth() / 7;
        System.out.println(thisWeek);
        System.out.println(pickedWeek);
        if(thisWeek != pickedWeek){
            System.out.println("im here");
            int day = cDate.getDayOfWeek().getValue();
            switch (day){
                case 1 : localDate = cDate.plus(4,ChronoUnit.DAYS); break;
                case 2 : localDate = cDate.plus(3,ChronoUnit.DAYS); break;
                case 3 : localDate = cDate.plus(2,ChronoUnit.DAYS); break;
                case 4 : localDate = cDate.plus(1,ChronoUnit.DAYS); break;
                case 6 : localDate = cDate.minus(1,ChronoUnit.DAYS); break;
                case 7 : localDate = cDate.minus(2,ChronoUnit.DAYS); break;
                default: localDate = cDate; break;
            }
        }
        else if(thisWeek == pickedWeek && localDate.getMonth() != cDate.getMonth() ){
            System.out.println("im here2");
            int day2 = cDate.getDayOfWeek().getValue();
            switch (day2){
                case 1 : localDate = cDate.plus(4,ChronoUnit.DAYS); break;
                case 2 : localDate = cDate.plus(3,ChronoUnit.DAYS); break;
                case 3 : localDate = cDate.plus(2,ChronoUnit.DAYS); break;
        ...
        for (int i = 0; i < day; i++) {
            pList.add(aService.getSingleDay(localDate,curGrade,homeClass).get(0));
            aList.add(aService.getSingleDay(localDate,curGrade,homeClass).get(1));
            tList.add(aService.getSingleDay(localDate,curGrade,homeClass).get(2));
